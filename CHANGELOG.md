...
- **`react-is` is the odd one out and stays in `dependencies`**, loosened from `^19.3.0` to `^19.0.0` so it deduplicates against the application's own copy (19.2.4 in the tree measured above) instead of installing a second one. It is genuinely imported and it is not a peer.
