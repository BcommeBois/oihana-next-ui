...
- **Two neighbours are deliberately left for their own lots** : `GridItem` destructures its properties explicitly and so does not receive these two, and [`@configs/ui/sidebar.js`](src/@configs/ui/sidebar.js) documents a `backgroundPattern` that nothing reads — the same defect, one floor up.
