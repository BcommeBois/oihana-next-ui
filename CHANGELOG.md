...
- **A round trip returns the same member.** The shape that was read is recorded in `properties.shape`, so a box comes back a box rather than the polygon it was drawn as. Float noise is trimmed to twelve significant digits — the figure `MetricScale` settled on — so `2.32` does not return as `2.3200000000000003` and diff against the stored value for nothing.
