...
- **MapLibre rather than Mapbox** : BSD-3 against a proprietary licence, no access token, no per-map-load billing, and a third of the weight. Leaflet was measured too and set aside — `react-leaflet` is published under a licence the OSI has not approved, and raster tiles cost the rotation, tilt, native clustering and hot style changes the rest of this roadmap needs.
