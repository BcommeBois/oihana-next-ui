...
- **New `contexts/previousPath`** (`context`, `provider`, `usePreviousPath`) : the URL of the page that was left, **query included** — a list holds its page and its criteria there, and the pathname alone sends a reader back to the first page of a list they had walked deep into. Published as state, so a consumer knows its destination while rendering rather than at click time. Mounted high enough to survive the navigations it observes ; outside it every consumer reads « fresh entry » and falls back on its own destination.
- **New `helpers/routes/resolveSmartHref( previousUrl , href )`** : the previous URL when it is the same page as `href`, `href` otherwise. The comparison is on the **pathname alone**, because the query is precisely what differs — and precisely what is worth carrying back.
- **`BackLink` takes `smart`** : the declared `href`, unless the page that was left IS that page, in which case its own URL is used. ⚠️ It replaces `history`, which is kept and documented as the weaker of the two : « the page rendered before » and « the previous entry of the browser history » part company as soon as a navigation replaces an entry rather than adding one, or a `back()` has already moved the cursor — the link then shows one destination on hover and goes to another on click, sometimes out of the application altogether.
- **`DisplayDropDown` takes `pageKey`** : the chosen mode is written to the URL (`urlParam`, `display` by default ; `null` remembers the mode without touching the address) and remembered for that page through `useDisplayPreference`. `value` stays an input, which a server render reads back from the parameter or the cookie. **The move is marked in place** and pushed without scrolling — a mode swapped over rows already on screen must not send the reader to the top, and `scroll : false` alone does not cover the shell's own reset. `useSearchParams` lives in that sub-component only, so a controlled dropdown never asks for a `Suspense` boundary.
- **`DisplayDropDown` fix** : its two effects sat AFTER the « nothing to switch to » early return — hooks called conditionally, which React throws on the first render where `modes` narrows to one. The guard now comes after every hook.
- **`PageHeader` takes `icon` as an already-rendered element**, as well as a component : a component type cannot cross the server → client boundary, an element can — so a Server Component renders its icon itself and hands the node over.
- **`PageHeader` takes `stackBelow`** (`sm` by default ; `md`, `lg`, `xl`, `2xl`, or `false` to keep one row) : below the breakpoint the right zone takes a line of its own. One row of three zones does not fold by itself — the title column is `min-w-0`, so its min-content width is zero and it keeps shrinking instead of pushing the actions down, and the title folded into four lines around them. Above the breakpoint nothing moves : left to wrap at every width, the row would send the actions down as soon as a description is long, on the widest screen — a layout moving for a reason the reader cannot see.
- Lab : a « Smart back link » card on the « Buttons » page (the remembered URL, exits carrying a query, the two links side by side with their resolved targets), a self-driving dropdown writing this page's own `?display=` on the « Layout » page, and `icon` as an element plus the three `stackBelow` settings on the « Headers » page. `PreviousPathProvider` is mounted in the lab's shell, above the pages, since it has to outlive the moves it remembers.
...
- **Reported from a consuming application** upgrading past 0.18.0 : about a dozen modals no longer opened at all. A click, and *nothing* — no exception, no log, no network trace. The modals mounted with the page and opened by a click went on working ; the ones that fell are those **mounted on demand** and opening themselves, which is the main recipe an application reaches for when a row opens its own editor or a route mounts a confirmation.
- **The cause is the mount gate `Portal` acquired in 0.18.0**, and the order React flushes effects in. On the commit that mounts the consumer, `<Portal>` renders `null` — so no `<dialog>`, and a ref still empty. React then flushes passive effects **children before parents** : the portal raises its own `mounted` flag, and only then does the parent's effect run `modalRef.current?.showModal()` on a ref that is still null. The re-render that finally mounts the dialog arrives after everyone who meant to open it, and a mount effect has `[]` for dependencies — nobody asks twice.
- **The optional call is what made it silent.** `?.` on a null ref is not an error, it is a no-op, so the failure produced not one line anywhere. That is also why it survived an upgrade unnoticed.
- **It had already claimed a victim nobody connected to it.** The popover fix shipped in the same 0.18.0 — every anchored dropdown invisible, its positioning effect reading a panel that did not exist yet — is this regression, one day earlier and on another ref. Two defects, one cause.
- **🚨 The hydration guarantee is kept, not revoked.** The 0.18.0 gate repairs a real inconsistency : a portal has no server-side form, and the hydration render has to say what the server said, or the client mounts an element where the server had sent none. What it should never have done is bill that to **every** portal on the page, when only the hydrating ones owe it.
- **`useSyncExternalStore` already knows the difference, and an effect cannot.** A flag raised from an effect answers `false` on the first render of *every* mount — it has no way to tell a hydration pass from a plain client mount. React does : it reads the *server* snapshot only while the fiber it is rendering is being hydrated, and the client snapshot otherwise. The new `hooks/useIsHydrated` is that, three lines, and `Portal` now asks it instead of an effect. So : `false` on the server and `false` again through hydration, the two renders that must agree ; **`true` on the first render of everything mounted afterwards**, with no commit spent and no frame lost.
- **A module-level flag was designed first and rejected**, and it is worth saying why. Set once from the first portal's effect, it is **global to the page**, while hydration is per root and per `<Suspense>` boundary — and under the App Router hydration is chunked. A boundary hydrating after some earlier portal has raised the flag would read `true` during its own hydration render, reintroducing the exact inconsistency 0.18.0 repaired. It also reads mutable module state during render, which concurrent rendering does not promise anything about. `useSyncExternalStore` draws the line where React draws it.
- **🚨 `useModal` carried the same silent failure, and no longer does.** `open()` returned on a missing node : an opening asked for before the modal had reached the DOM was dropped without a word. The intention is now **held and replayed** — the hook's ref setter, already the smart accessor that re-attaches its listeners on a node swap, opens the node the moment it is handed one. An opening is late at worst, never lost. `close()` called in the meantime calls it off, and `lock` is honoured at replay time rather than at request time.
- **`openOnMount` is that same intention, pre-loaded.** `useModal({ openOnMount : true })` opens whatever node it is eventually given, whenever that is, and the component holding it needs no effect and makes no call. It is `false` by default, read once on mount, and it is the right spelling for a modal whose *mounting is* the request to open it. Where a portal cannot exist at all — a modal present in the server render — it is the only spelling that works, the memorisation being what bridges the gap.
- **🚨 `Modal` was remounting its own `<dialog>`**, a variant of the same defect hiding behind a gate of its own : `portal && mounted` put a bare `<dialog>` at a position on the first commit and a `<Portal>` at that same position on the second. Two element types, one position — React unmounts the node and mounts a new one, the ref detaching and re-attaching with it, and an opening played on the first node dies with it. This one failed *with the ref attached*, which is the more deceiving of the two symptoms. `Portal` holding the gate for everyone, `Modal`'s own is gone : one element type, one mount, one ref.
- **A portalled `<Modal>` now renders nothing server-side**, where it used to emit its `<dialog>` in place, closed and invisible, before moving it. Nothing changes visually — a portal cannot run on the server, and this is the consistent form of saying so. Consumers opening such a modal imperatively from a mount effect should go through `useModal`, which waits for a node ; a raw `showModal()` on the DOM node cannot.
- **`Popover` keeps its callback ref**, and only its comment changed. It reads as a workaround for a portal that mounts late, and it is no longer only that : measuring on `isOpen` is measuring on the wrong signal in the first place — the two coincide only when the panel mounts in the commit that opens it, which hydration still breaks — and detaching the ref is what drops the stale measurement so the next opening cannot paint at the previous trigger's position.
- **🚨 The lab did not exercise the recipe anywhere**, which is the whole reason this reached a release. `showModal` appeared in no demo : every modal on the page was mounted with it and opened by a click, and the on-demand mount that opens itself — the shape consuming applications actually write — was demonstrated nowhere. `/lab/modals` now closes on **Mounted on demand, opened on mount** : four cards, each isolating one mechanism (a hand-written `Portal`, the `portal` prop, `open()` from the hook, `openOnMount`), each reporting whether the node existed when the effect ran. Written before the fix and verified failing on all four, so it is a test and not a decoration.
...
- **🚨 The cause is a range conflict, not a precedence between blocks.** `next`, `react` and `react-dom` were declared in **both** `dependencies` and `peerDependencies`, but the duplicate alone produces nothing : what nests a second copy is the two blocks disagreeing with what the host installed. `react` was asked as `^19.3.0` against an app on 19.2.4, and `next` was **pinned exact** (`"16.2.3"`) against an app on 16.2.11. An application that happened to sit on react 19.3.0 and next 16.2.3 would have had no nested copy at all, duplicate declarations or not. **The 293 MB is therefore not a constant** : it is the bill for version drift, and nothing was keeping it from growing.
- **The real defect was never the disk.** It is a second copy of the framework in the consumer's tree — the same class of bug as the duplicated `@nivo/core` already told here, where two copies mean two sets of React contexts that break in silence. It was harmless by the single grace of Next's aliasing : under the App Router *the React that runs is the one Next bundles*, which the 0.18.0 entry already states.
- **Except that nothing aliases `react-is`** — and the honest version of that exception is narrower than it looks. `react-is` is imported by exactly one file, `helpers/react/isElementType`, which **nothing in the library imports** : it is a public entry point and only that, like `helpers/date/parseISO` one entry below. Since the library publishes raw source, a consumer's bundler resolves it only if the application imports that helper itself. So the nested 19.3.0 was always *installed*, and *bundled* only in that one case — where it would have been a table of pure functions, without consequence.
- **🚨 Loosening the `next` peer is the load-bearing half of the fix, not a tidy-up.** Removing the three from `dependencies` while leaving `"next": "16.2.3"` exact in `peerDependencies` would have left the 285 MB exactly where they were : npm ≥ 7, pnpm and bun all auto-install peers, as this changelog notes about the nivo peers, and an app on 16.2.11 does not satisfy an exact 16.2.3. It is now **`^16.0.0`**, which is what the README already claimed and what the imported API actually needs — `Image`, `Link`, `usePathname` / `useRouter` / `useSearchParams`, `cookies()`, `revalidatePath`, `next/font/google`, nothing newer.
- **A peer floor is a minimum, not a current version.** Setting it to the latest Next would be this same bug turned round : `^16.3.5` would leave an application on 16.2.11 unsatisfied and reinstall the 285 MB through the other door. `react` and `react-dom` stay at `^19.0.0` for the reason documented in 0.18.0 — `ViewTransition` is reached through the namespace, so an earlier React degrades instead of crashing.
- **`react-is` is the odd one out and stays in `dependencies`**, loosened from `^19.3.0` to `>=19.0.0` so it deduplicates against the application's own copy (19.2.4 in the tree measured above) instead of installing a second one. It is genuinely imported and it is not a peer. The `>=` spelling is deliberate twice over : `react-is` tracks React's major, so a consumer on a later React should be met there too — and `bun update` rewrites a caret range onto the resolved version while leaving this one alone, which is what keeps a floor a floor.
- **The three go back into `devDependencies`** — peer **and** dev for a non-optional peer, which is this repository's rule and the one the charts entry caught itself breaking on `@nivo/arcs`. `daisyui`, `tailwindcss` and the `@nivo/*` were already treated that way. Without it, this repository being itself a Next application, `bun dev` and `bun build` stop working here.
- **🚨 It survived eighteen minor releases because it costs nothing here.** A library's dependency blocks only misbehave inside a *consumer's* `node_modules` — the one tree this repository never looks at, its own being correct whatever the manifest says. So `scripts/check-deps.js` now refuses two things : a package declared in both `dependencies` and `peerDependencies`, and a non-optional peer absent from `devDependencies`. It runs as `preversion` and **exits non-zero**, unlike `cache-size` : this is a contract, not housekeeping. Pointed at the manifest as it stood, it reports the six failures above.
- **`ky` was in `dependencies` and imported nowhere**, so every consumer installed it for nothing. Removed. The only `ky` left in `src` is the string `'ky-KG'` in a list of locales.
- **The README said what to depend on without saying who provides it.** A line under `bun add` now does, and the requirements block says out loud that the application must already carry them.
- **A patch is enough.** No consumer can be missing what is already its peer, and every package manager installs missing peers by itself. The nested tree only disappears after a published release and a bump on the consumer's side, most likely with `node_modules` wiped since `bun.lock` holds the resolution — **so nothing here proves this fixed**, and the lab starting is necessary, not sufficient.

**The lab moves to Next 16.3, and `cache-size` grew a blind spot the same day**

- **16.2.3 dated from April and was not even the last of its own minor line**, which ended at 16.2.12. `devDependencies` takes `^16.3.5`. **Nothing changes for consumers** : the peer is `^16.0.0` and already welcomed it.
- **16.3 enables Turbopack's filesystem cache for `next build` by default** — `next dev` has had it since 16.1. So there are two caches from now on, `.next/dev/cache/turbopack` and `.next/cache/turbopack`, and `cache-size` named neither the second nor anything outside `dev/`. It would have gone on weighing `.next` whole, reporting a total its own breakdown no longer explained, warning on nothing — and **recommending a `clean:cache` that deleted only the other folder**.
- **Measured here the day it was written : the dev cache is at 8.1 GB against a 5 GB limit.** The alarm is already ringing, and a second tree of the same nature was about to appear where nothing was looking.
- **🚨 One limit per folder, not one limit over their sum.** The two do not mean the same thing : the dev cache accumulates across weeks of sessions — that drift is what this script was written to catch — while the build cache is rewritten by every build, so an identical figure can be perfectly ordinary there. Summing them would sound the alarm on a healthy build cache and drown a dev cache going wrong, and the report has to say *which* folder to empty, since they are emptied separately. **The build limit is provisional**, borrowed from its neighbour rather than measured : that folder does not exist here yet.
- **A cache that does not exist is left out of the report entirely**, rather than printed at zero. Until Next writes the second one, the output is what it always was, to the byte.
- **`clean:cache` empties both**, and the warning prints the exact `rm -rf` for the folder at fault.
...
**Three pickers that select a period without ever showing a day — `YearPicker`, `MonthPicker`, `MonthYearPicker`**

- **The two grids were already written, they simply did not own anything.** `MonthsGrid` and `YearsGrid` have been the calendar's quick navigation since the start — twelve localised months, twelve years paged ±12 — but their `onPick` moved the view rather than producing a value, and neither was reachable from outside `Calendar`. The new components are those same grids given a value, so a month or a year is picked with exactly the cells the calendar already taught.
- **One component per granularity, each with a value type that cannot be misread.** `YearPicker` hands back a `number`, `MonthPicker` a month index `0`–`11` with **no year attached** (a recurring month, an anniversary, a season), and `MonthYearPicker` a `Date` on the first day of the month. A single component behind a `precision` prop was the other candidate and was dropped for the reason that would have been paid at every call site : the type of `value` would have changed with the prop.
- **🚨 In `MonthYearPicker`, only the month commits.** Clicking the year in the header opens the 12-year grid and picking one carries the picker to that year's months — the same chain `Calendar` walks, one step shorter (years → month, where the calendar goes years → month → day). A year alone is an incomplete answer to *which month*; when the year **is** the answer, `YearPicker` is the component.
- **`min` / `max` take a `Date` or a year, and a year bound covers the whole year** (`helpers/date/periodBounds`) : `max={ 2030 }` leaves December 2030 selectable rather than blocking everything after the 1st of January. As in the calendar, **only the bounds stop navigation** — a year greyed out by `disabledYears` stays reachable, otherwise the year behind it would not be.
- **The grids gained `showHeader`, `columns` and `labelFormat`, all additive.** Without a header the months grid is a bare month selector ; without `onYearClick` its year is a plain label instead of a button ; `columns={ 3 }` and `labelFormat="MMMM"` are what whole month names ask for. `Calendar` passes none of them and renders exactly as before.
- **The column count goes through `getCalendarGridClasses`**, whose map holds `grid-cols-2` / `-3` / `-4` / `-6` as whole literals — a count that is not one of those falls back to four rather than inventing a class the scanner never saw.
- **They are plain blocks, and closing on pick is the caller's decision.** A picker that dismissed itself could not be used inline, so the three have no opinion about where they live : a page, a card, or a `Popover` — dropdown on `md`+, bottom sheet below — where the caller's `onChange` is what closes it. `role="group"` with an `ariaLabel` names the grid ; a `<fieldset>` was refused on purpose, being a form control with a layout of its own.
- **`/lab/dates` shows the three inline**, with bounds, blocked periods and full month names, then the same two in a popover and in a forced modal.
...
- **🚨 Off by default.** `animate` is a change of behaviour and the arrows work the same without it, so it is opted into rather than out of. Turned off, no `startTransition` is issued at all : nothing is marked as a Transition, so nothing can animate — the wrapper is a plain `Fragment`.
- **🚨 One stable region with `update`, not a wrapper per month.** The obvious shape — `<ViewTransition enter exit>` around each keyed `MonthGrid`, since a month change already unmounts one and mounts the next — **animates nothing at all**. Instrumenting `document.startViewTransition` in the browser showed the transition starting with the right type and **no element tagged** : React does not name a `<ViewTransition>` that comes and goes at a list position. What it animates is a region that *stays* while its content changes, which is what `update` is for. Everything downstream of that mistake looked like a CSS problem and was not.
- **🚨 The two months must not share the screen.** Both snapshots are stacked in the same box, so a short travel makes them read as one double-exposed month rather than as a movement. The old one leaves first (130 ms) and the new one waits for it — `both` holds the opening keyframe through the delay, so nothing is ever painted twice — and the travel is 40% of the width rather than a couple of rem. The group clips, so the journey stays inside the calendar.
- **The quick month and year pickers stay out of it**, deliberately. Jumping to March 2031 from a grid of years is not a step forward or back, and sliding it one way or the other would say something untrue.
- **🚨 Reached through the namespace, so an older React degrades instead of throwing.** `ViewTransition` landed in 19.3 and this library's peer range is `^19.0.0` — a *named* import of what a module does not export is `undefined`, and rendering `<undefined>` throws. `helpers/react/viewTransition` hands back a pass-through component instead, which is also what receives and ignores the props a `Fragment` would have warned about. No peer bump : in a Next App Router application the React that runs is the one Next bundles, and Next 16.2 ships a 19.3 canary whatever `package.json` declares.
- **`prefers-reduced-motion` fades rather than travels** — the change is still seen, the journey is not. The rule is scoped to this component's two classes : a page's own view transitions are none of our business.
- **The CSS ships with the component** (`dates/styles/calendar-transition.css`), as the spinners' does. React writes the name from `update` into the element's `view-transition-class`, so each side is selected through `*.<name>` on the pseudo-elements.
- **`/lab/dates` shows the two side by side** — same calendar, one prop apart.
...
- **🚨 A portal has no server-side form, and saying so on the server alone is not enough.** The `if ( !container ) return children` sitting underneath looked like the right fallback and is not : rendering the children *in place* puts them where the client is about to move them away from. But returning nothing server-side still leaves the **hydration** render free to portal immediately, which is the same disagreement one step later — the client mounting a `role="tooltip"` element where the server sent none.
- **So the target is only reached once mounted**, which is the pattern `Modal` has carried from the start and which is now where it belongs — in `Portal` itself, for everything that portals. Server, hydration render : nothing. Then the Effect runs and the content goes where it goes. It costs one frame for the only case that renders a portal from the first paint — a tooltip held open — and nothing at all for the four that wait on a click.
- **The in-place branch stays for the one case it does answer** : a `containerRef` whose node is not attached yet.
...
- **`markerProps` is how a marker learns what it is** — it receives the source object and returns props merged into the `MapMarker`. The component stays `items`-driven, as everywhere else in this library, while the mapping from a business type to a colour stays in the application, which is the only place that knows it.
- **Clustering is off by default.** It changes *where the points appear*, and that is not a decision a component should take on a caller's behalf. The demo's own toggle is the argument : the same two hundred sites read completely differently with it on.
- **🚨 `supercluster` rather than the engine's own clustering, and the reason is testability.** MapLibre can cluster a GeoJSON source — it embeds this very algorithm — but reading the result back means `querySourceFeatures`, which only returns what is already in rendered tiles : the answer depends on the state of the renderer, needs a layer (even an invisible one) for the tiles to load at all, and cannot be exercised outside a browser. Called directly, in plain JavaScript, the same algorithm is deterministic and answers before anything is drawn. That is what keeps the bubbles DOM elements styled from the theme instead of a paint spec. A third optional peer, at 3 kB gzipped.
- **`Map` now publishes its instance through a context of our own.** The engine has one and does not export it, and its public `useMap` only answers under a `MapProvider` the consumer would have to mount — which would be a leak : `<Map><MapMarkers/></Map>` has to be enough. Everything that needs to *move* the map reads it from one place, and the geolocation control and the picker will read the same one. **The hook does not reach for it** : `MapMarkers` reads the context and passes the instance in, because a hook depending on a component is a direction nothing else in `hooks/` takes, and because grouping should not require being inside a map to happen.
- **The viewport is read, never driven.** The map stays uncontrolled ; the cluster hook subscribes to `moveend` and asks where it currently is. The instance is published on `load` rather than on mount, which costs one paint before the first bubbles appear and buys a map that is actually ready to be moved.
- **A bubble is a real button, and its name says what it does.** « 24 sites, zoom in » rather than a circle with a number, which means nothing to a screen reader. Past a thousand it prints `1k+` : nobody reads `1 248` off a 56-pixel circle, and the bubble would have to grow to hold it.
- **Three bubble sizes, not a computed diameter.** A size built from a count never appears in the source and Tailwind only ships what it can read — and three steps is as much as the eye reads reliably anyway. A radius tracking the count exactly would say « slightly more » where the reader only needs « more ».
...
**🚨 A spread point was leaking into the DOM, and had been since the first lot**

- **`<Map { ...fromSchema( place ) } />` is the advertised shape**, and the price of it is that everything the point carries besides its two coordinates arrives in the rest object : `elevation` and `source` from the adapter, `accuracy`, `heading`, `speed` and five more from a browser fix. They were being spread onto the frame's `<section>`.
- **It had been silent for two lots.** React complains out loud about a camel-cased attribute — `altitudeAccuracy`, which is what surfaced it — and says *nothing at all* about a lower-cased one. So every map drawn from an adapted place has been shipping `elevation="20"` in its markup since the first commit, with no warning anywhere.
...
**A bubble on the map, and a search field the map carries**

- **`MapPopup` is anchored to coordinates, not to an element**, which is the whole difference with `Popover` : during a pan the marker moves continuously, and only the engine can keep a bubble glued to its ground. It answers *what is this point* ; a modal answers *let us work on this record*, and that already worked — `onClick` has been on `MapMarker`, `MapCluster` and a route's stops since their own lots.
- **Anything goes inside it.** The engine's padding is removed so the content decides its own, which is what lets a `Card` sit in a bubble without a frame inside a frame. `anchor`, `offset`, `maxWidth`, `closeOnClick`, `closeOnMove` and two class props cover the rest, and the close cross is the one `Modal` and `Popover` use rather than a third that looks almost like them.
- **🚨 Theming it needs `!`, against the habit of this codebase.** The rule everywhere else is that a Tailwind utility beats DaisyUI — true because DaisyUI nests its rules inside `@layer utilities`, where unlayered content wins. **MapLibre's stylesheet is not layered at all**, so its declarations beat *every* layer whatever the specificity, and an ordinary utility loses : the popup would stay white on a dark theme. Every class in `getMapPopupClassNames` carries the important modifier for that reason, and it is the first place in this project where the rule runs the other way.
- **The tip is eight classes, not one.** MapLibre colours the little triangle by setting a single border side per anchor — bottom when the bubble hangs below the point, top when it sits above, and so on. Each has to be answered on its own side ; setting all four would draw a square.
- **`MapSearch` is the lot 5 field in the lot 3 control**, and nothing more. The geocoder stays injected, so the same component searches addresses at a service or **your own records** — both answer `Place` objects, and choosing which is not the library's business. That was the most interesting part of the idea and it cost nothing.
- **🚨 Two shapes, because the frame clips.** `MapFrame` is `overflow-hidden`, so on a short map the suggestion list is cut off at the bottom edge — five suggestions do not fit under 320 pixels. `inline` suits a wide map ; `modal` opens its dialog in the browser's top layer, which no `overflow` can reach, and is what a short map or a phone needs. The demo puts a wide one and a deliberately short one side by side so the constraint is visible rather than asserted.
...
- **🚨 A circle has no inside, so it becomes a polygon.** `GeoShape.circle` is a centre and a radius, and `parseGeoShape` keeps it that way — faithful to schema.org and impossible to fill. `toCirclePolygon` approximates it in *real coordinates*, so it grows with the zoom and survives a rotation or a tilt ; a disc sized in pixels, as the geolocation accuracy circle is, would do neither.
- **The approximation is measured, not asserted.** Against haversine distances : 0.11 % at 1.5 km, 0.06 % at 50 km, 0.23 % at 100 km, and 0.08 % at 5 km on latitude 68 — so the error grows with the radius, not with the distance from the equator, which the cosine already handles. Two metres on a two-kilometre delivery radius. Past a few hundred kilometres a geodesic library is the right tool, and the JSDoc says so rather than pretending otherwise.
- **🚨 `toGeoShape` is the way back, and it ships now rather than with the drawing lot.** Without it a zone could be displayed and never saved, and the inversion between schema.org's `latitude longitude` text and GeoJSON's `[ longitude , latitude ]` would run in one direction only. It lives in `parseGeoShape`'s own file so the two halves are read side by side — anywhere else, one could be corrected without the other.
- **A round trip returns the same member, and the same values — not the same text.** The shape that was read is recorded in `properties.shape`, so a box comes back a box rather than the polygon it was drawn as. Float noise is trimmed to twelve significant digits, the figure `MetricScale` settled on, so `2.32` does not return as `2.3200000000000003`. But **trailing zeros do not survive** : `48.900` comes back `48.9`, because once the text is a number the two *are* the same number and nothing says how many decimals to write. A store detecting changes by comparing strings will therefore see an untouched shape as modified — compare numbers, or compare against what a round trip produces.
- **A `MultiPolygon` is a known gap**, written down rather than discovered later : `MapGeoJSON` filters its fill on `Polygon`, so a multi would be outlined and not filled.
- **The drawing half is a lot of its own.** `terra-draw` and its MapLibre adapter are two more optional peers, an editing state machine and an ergonomic decision per mode. Shipping the display first is what lets the format be checked before anything writes it.

**Routes — and the layer path the earlier lots kept deferring**

- **🚨 The vector-layer problem turned out to have been solved for a year.** It had been deferred twice on the grounds that a layer is styled in the engine's paint spec, out of Tailwind's reach, so theme tokens would have to be read at runtime and re-injected on every theme change. That mechanism is `useThemes().colors` — the theme's colours extracted from the CSS variables — and it is what `usePalette` has fed the charts from the start. A layer reads the token through it, the theme changes, the layer repaints. Nothing had to be invented.
- **`MapGeoJSON` is the primitive**, and routes are its first passenger — the same order as `MapControl` before it. Zones will sit on it in the next lot, and `render="layer"` for markers has a place to go the day a dataset asks for it.
- **`MapRoute` draws a geometry and never computes an itinerary.** Given one it draws the road, solid ; given none it joins the stops with straight **dashed** segments, the dashes saying *this is an order of passage, not a road*. A solid line there would be a lie about a path nobody computed, and the toggle in the demo puts the two side by side.
- **The order comes from the data, through an accessor rather than a guess.** `helpers/geo/fromRoute` reads `position` off the stop and takes an accessor for a payload that nests it — because `DeliveryRouteAssignment` carries the rank but *not* the place it applies to : the address holds the assignment, not the reverse. Inventing a property name here would work until the first API that nests it differently.
- **A colour carried by the data reaches everything.** A `DeliveryRouteTerm` composes `HasColor`, so its hex goes to the line and to the markers alike — `MapMarker` gaining the `background` prop `MapCluster` already had, painting inline and computing its text by contrast. A theme token still takes the class path ; the component tells them apart by membership in the token list rather than by sniffing at the string.
- **`fit` brings the map to the whole route**, because a route almost always overflows the opening view — the lesson the address search taught, applied before it could be reported again. Only one route per map should carry it : two would fight over the view, and the demo says so where it puts two on one map.
- **No direction arrows.** They need a symbol layer with `symbol-placement`, an embedded icon and a size that survives zoom — and the numbered stops already say which way round it goes.

**Searching for an address, without the library choosing a provider**

- **`InputAddressSearch` takes a `geocode` function and nothing else.** Which provider, in which country, under which terms is an application's decision — the component never imports one. The contract is `( query , { signal } ) => Promise<Place[]>`, schema.org objects, so `fromSchema` reads the answer with no translation and a selection hands back the whole address rather than only its point.
- **🚨 The abort signal is part of the contract, not an optimisation.** Without it a slow first request lands after a fast second one and the list shows results for what was being typed two words ago. Every superseded request is cancelled, which is the only arrangement that *cannot* show a stale answer — filtering late responses by comparing queries only narrows the window.
- **One provider is shipped, as an adapter nothing imports on its own** : `helpers/geo/adapters/ban`, thirty lines against `api-adresse.data.gouv.fr` — free, no key, no account, and better on French addresses than any international service. Useless outside France, which is exactly why it is an adapter and not a default. Writing it once here beats writing it again in every application.
- **🚨 The BAN answers GeoJSON**, so its coordinates arrive as `[ longitude , latitude ]`. That inversion is one line of the adapter and it is the only line that matters : read in the wrong order, every French address lands in the Indian Ocean. It joins `parseGeoShape` as the second and last place in the library that touches an axis order.
- **It is a real combobox, and the library had none.** `aria-expanded`, `aria-controls`, `aria-activedescendant`, arrow keys that wrap both ways, Enter for the highlighted one, Escape to close, and focus that never leaves the field. The options are deliberately **not** focusable — that is what the activedescendant pattern *is*, and the linter's objection to it is suppressed with that reason rather than obeyed.
- **It builds on `Input`, not on `InputSearch`.** That one owns the Enter key for its own search, and a combobox needs Enter to mean « take the highlighted suggestion » : building on it would have fired a search on every selection. The debounce it also brings is one hook away.
- **`InputGeoPoint` grows a search field when given a `geocode`**, and that is what makes it usable in a form at all — nobody knows their coordinates, they know their address. Type, take a suggestion, then drag the marker because the geocoder landed in the middle of the street. Without a `geocode`, nothing changes.
- **🚨 The map now follows a value it did not cause.** The previous lot had said, flatly, that the map does not chase its value — right for a drag, where pulling the ground from under the hand doing the dragging is unusable, and wrong for everything else. Choosing an address in Amiens while the map showed Paris dropped the marker off-screen and looked like nothing had happened. `MapPicker` remembers what its own drag last emitted and flies to anything else, `flyTo` rather than `easeTo` because an address can be a hundred kilometres away and panning across at ground level reads as a glitch. The comparison is by value, not by identity : a host is free to clone the point on its way through a reducer.
- **Four states, not one** : idle, searching, no results, and the geocoder failed. A field that goes quiet when a service is down teaches people to retype the same address.
- **No reverse geocoding.** Dragging the marker does not update an address — it is a second call, a second state and a second contract, and it deserves its own lot rather than a corner of this one.
...
- **The position comes back flat, `latitude` and `longitude`** — the same names `fromSchema` returns, so a fix spreads straight into a `Map` or a `MapMarker` with nothing in between. The demo shows exactly that, and it is the third component in a row to get it for free.
- **🚨 Nothing happens until you ask.** Mounting reads the *permission state*, which does not prompt ; the browser dialog only appears on `request()`. A hook that asked on mount would put a permission prompt in front of anyone who merely loaded a page — the one behaviour that teaches people to click *block* and never revisit it.
- **A refusal is a state, not an error to swallow** : `isDenied`, with an `error.kind` of `refused`, `unavailable`, `timeout` or `unsupported`. The demo gives each one a sentence, because a button that silently does nothing when pressed is the worst of the outcomes. The permission query does not fire on a refusal in every browser, so the error handler sets the state itself rather than leaving a caller to know that.
- **🚨 The map's control is ours, after the engine's proved impossible to fire.** The plan said to wrap MapLibre's `GeolocateControl` rather than reimplement its accuracy circle and tracking. Mounted, it rendered an enabled button carrying every attribute its own setup function writes — including the one written on the same line as the click listener — and neither a human click nor a programmatic one produced any state change. Four rounds of bisection through the minified source ruled out the disabled path, the module-level support cache and a duplicated button, and left two facts that cannot both be true under any reading of that code. So it was dropped rather than understood.
- **`MapGeolocate` is a `Button` in a `MapControl`, on `useGeolocation`.** It takes the theme, its behaviour is ours to change, and a future control sits beside it. That is worth more than a borrowed control we cannot open — which is also the argument that had been made *against* writing it, and it did not survive contact.
- **Two states rather than five.** Idle and locating, with `track` turning the button into a toggle. What is deliberately missing is the engine's third state — the watch running while the user has dragged the map away, so the button changes shape and one press re-locks. It costs two more appearances and is understood by nobody who sees it.
- **🚨 The accuracy circle needed no vector layer**, contrary to what the plan claimed when it deferred it. MapLibre draws it as a plain element and resizes it on every scale change : project the centre, step a hundred pixels east, ask how far that is in metres, divide. Four lines. The deferral was based on a wrong reading, so the circle ships in this lot — a translucent disc in the theme's colours, on by default, because a bare dot on a five-kilometre desktop fix reads as a certainty.
- **`MapControl` is the primitive underneath**, and geolocation is only its first passenger. Four corners, `top-left` by default since the engine holds the top-right, and `bottom-right` pushed up because the source credit lives there. It wraps its children exactly, so the map stays draggable everywhere it does not cover.
- **`helpers/permissions` finally has its `geolocation` entry**, which was the one name missing from a list that already knew about the accelerometer.
...
- **The precedent decided it** : `charts` has four pages under its own group, `scheduler` six. A lone entry suits a component ; maps are a family of nine lots, and moving them now costs a wiring change, later a habit.
- **`mapsModel` mirrors `schedulerModel`**, which carries exactly the same thing for the other family — the vocabulary and the adapter that reads it. The parallel is deliberate.
- **`/lab/maps` stays a valid URL**, so nothing anyone bookmarked breaks.
- **The palette label was the wrong size and wrapped over three lines** — its wrapper was missing daisyUI's `label` class, without which `label-text` is not styled at all and falls back to the base size. Its neighbour had it, which is what made the difference visible.
- **The demo crowd is generated from a seeded sequence**, not `Math.random` : the server and the client have to lay out the same two hundred and forty points. They gather around three centres rather than spreading evenly, because an even scatter clusters into a uniform grid — which shows the mechanism and hides what it is for.

**`helpers/geo` — the adapter, written before the first map**

- **Four functions, no dependency at all**, so they can be read, checked and corrected before anything renders : `fromSchema`, `parseGeoShape`, `toGeoJSON` and `formatCoordinates`. The day the engine changes, this half does not move — which is the whole reason it was built first.
- **🚨 A `Place` carries its position twice**, in `geo` and in its own flat `latitude` / `longitude`, and both are legitimate schema.org. A view that takes whichever it finds first draws the same place in two spots depending on which emitter produced the payload. `geo` wins by default, and `fromSchema( place , { prefer : 'flat' } )` inverts it.
- **🚨 `GeoShape` is not GeoJSON, and this is the only file that knows it.** Its `polygon`, `line`, `box` and `circle` are *strings* of space-delimited pairs written `latitude longitude`, where GeoJSON wants `[ longitude , latitude ]`. The swap raises nothing — it draws the right shape in the wrong country — so the inversion happens in `parseGeoShape` and nowhere else. Everything around it names its coordinates, and named coordinates cannot be swapped by accident.
- **The names are `latitude` and `longitude`, not `lat` / `lng`.** It keeps the schema.org vocabulary end to end, and it happens to be what `@vis.gl/react-maplibre` takes too — so a point spreads straight into a component, `<Map { ...fromSchema( place ) } />`, with no axis named twice between the payload and the screen.
- **Nothing reads `@type`.** An object carrying a coordinate pair is a point whatever it calls itself, so a house subtype works without being declared on this side — the rule the scheduler's adapter already follows.
...
- **`Map`, `MapFrame`, `MapMarker` and `MapControls`**, on `maplibre-gl` and `@vis.gl/react-maplibre` — both **optional peer dependencies**, declared in the three places the rule needs, so nothing is installed for anyone who does not draw a map.
- **MapLibre rather than Mapbox** : BSD-3 against a proprietary licence, no access token, no per-map-load billing, and 268 kB gzipped against 490 — near enough half. Leaflet was measured too and set aside — `react-leaflet` is published under a licence the OSI has not approved, and raster tiles cost the rotation, tilt, native clustering and hot style changes the rest of this roadmap needs.
- **🚨 `maplibre-gl` is pinned to the 5 line, and the reason is a defect worth writing down.** Version 6 splits its web worker into a separate `maplibre-gl-worker.mjs` and resolves it as a *sibling of `import.meta.url`* at runtime. Under a bundler that relocates the chunk — Turbopack here — that sibling does not exist : the request 404s, the dev server answers with its HTML page, and the browser refuses it with `non-JavaScript MIME type of "text/html"`. The failure is the worst kind : the map mounts, the controls and the credit appear, and not one tile is ever drawn. No bundler configuration fixes it, the URL being computed from a string at runtime and therefore invisible to static analysis. Version 5 inlines its worker as a blob and has no such file. The peer range says `^5.24.0` rather than `>=5`, because a range should say what is actually supported.
- **`@vis.gl/react-maplibre` directly, not `react-map-gl`.** The meta-package declares *both* `@vis.gl` wrappers in its `dependencies` — not its peers — so it would ship the Mapbox wrapper for nothing. The twin is the same version, from the same team, with the same API.
- **`MapFrame` is `ChartFrame`, prop for prop** — `aspect` over `height`, `maxHeight`, `loading`, `empty` / `emptyLabel` / `emptyState` — because a map has the same box problem and the same two lifecycle states, and there was no reason to invent a second vocabulary for them.
- **🚨 The attribution belongs to the engine, and the frame stays out of the way.** MapLibre's control reads the credit from the style's sources **and from the TileJSON they point to** — which is where a provider actually declares it : OpenFreeMap's style file carries none, and the line comes from `tiles.openfreemap.org/planet`. So the frame prints nothing unless it is given something, and `attribution` on `Map` says who prints what — omitted for the engine's responsive default, `'compact'` to collapse it to a button, `true` to force it open, a node to replace it, `false` for neither. `false` is allowed because a page may credit the data elsewhere ; ODbL still requires it somewhere.
- **The engine is imported in exactly one file**, `maps/engine`, which is also where its stylesheet is pulled in. Reading the library showed `next/dynamic` was the wrong reflex there : `Map` touches no browser global while rendering and only renders its children once the instance exists, so it is already SSR-safe ; `maplibre-gl` is already loaded by the library's own `import()` inside its mount effect, so the 139 kB is already split out ; and `next/dynamic` does not forward refs, which would have thrown away the map handle a picker needs.
- **Markers are DOM, not a vector layer.** A layer holds tens of thousands of points but its look lives in the engine's style spec — out of reach of Tailwind and of the theme tokens. In the DOM a marker takes `color` and `size` like a `Badge`, follows the theme into dark mode, and can be replaced outright by `children`. The trade is the count : a few hundred, not a few thousand.
- **A clickable marker is a real `<button>`**, named by its `title` ; one without a handler is `role="img"` with the same name. A marker whose only label is its colour says nothing to a screen reader.
- **A named frame renders as a `<section>`**, which is a region once it has an accessible name — where `ChartFrame` uses `role="img"` to collapse a chart into one labelled image. A map is the opposite of a chart here : it pans, zooms and takes keyboard focus, and `img` would hide every one of those controls.
- **`controls` is `true` by default and `true` means navigation only** — zoom and compass. A map with no way to zoom is not a simpler map, it is a broken one. The scale bar and the fullscreen button are opt-in, and an object is merged *over* the default rather than replacing it, so `{ scale : true }` adds to the navigation instead of trading it away.
- **`bounds` names its four edges** — `{ north , south , east , west }` — rather than taking two coordinate arrays. Four numbers in an order nothing on the page states is how a valid map of the wrong place gets drawn.
- **The viewport is uncontrolled**, deliberately : the centre and zoom say where the map opens, and changing them does not move it. That is right for showing a place and wrong for editing one, so the ref exposes the map instance and a controlled viewport waits for the lot that needs it.
- **`mapStyle` already accepts `{ light , dark }`** although only one half is used yet. The map will follow the theme eventually, and the prop should not have to change shape when it does.
- **`mapStyle` is required, and a missing one is its own empty state** — `No map style`, distinct from `No location`. One sends the reader to the configuration, the other to the data.
- **The style URL is read by the application, never by a component.** `NEXT_PUBLIC_MAP_STYLE` reaches the lab through `@configs/ui/map`, and a tile key is public whatever anyone does about it : it leaves from the browser with every request, so what protects it is being restricted to a domain, not being hidden. The committed `.env.example` carries the keyless OpenFreeMap default and says so.
- **A lab page at `/lab/maps`**, under *Display*, with the maps above and the adapter printed as plain text below — three payloads and the figures they yield. A swapped pair draws a perfectly good map, so reading the numbers is the only way to catch it before the map hides it.
- **A guide in `wiki/components/maps`**, with the install matrix, the whole "do you need an account" question, and what to do about a key when you do have one.
...
- **`ignore` names the parameters that do not move the page**, and `Drawer` forwards it through a new `scrollReset` prop — `{ behavior , disabled , ignore }`, or `false` for the blunt all-or-nothing that was the only lever before. `disabled` alone was never the right granularity : the question is not whether to reset, it is *which parameter changed*.
- **The comparison is a signature string, not the objects**, and that is what makes the filter possible at all. `useSearchParams` hands back a fresh instance on every navigation, so the effect fired even when nothing it cared about had moved — the identity was the real defect under the reported one. Comparing `pathname` plus the sorted entries minus the ignored keys fixes both at once.
- **🚨 A navigation is ignored only when *everything* that moved is ignored**, and the signature gives that rule for free. One control can write a parameter and drop others in the same push — a year picker resetting an offset, a filter clearing a page number — so ignoring the one it writes is not enough : the dropped key leaves the signature too, and the reset fires unless it was on the list as well.
- **The list enters the memo joined into a string.** `ignore={ [ 'topYear' ] }` is a new array on every render, and left as an array in the dependencies it would rebuild the signature every time — the very defect being cured, met from the other side.
- **The hook now remembers the signature it last acted on**, which stops it firing when one of its *own* options changes mid-life. It used to send the reader to the top of a page that had not gone anywhere. The record is kept even while `disabled`, since switching the reset back on is not a navigation.
- **`useResetScroll` is a public export, so its old call shape still works** — `( ref , disabled , scrollClassName , behavior )`, deprecated in the JSDoc, kept for one minor. The options object is the form to write now.
- **The mount-time reset stays, deliberately** : a first paint at the top is right.
- **🚨 The ignore list belongs to the shell, and a page cannot set it** — which is why `Dashboard` now forwards a `scrollReset` too, and reads one from `ui.dashboard` beside `breakpoint` and `layout`. `useResetScroll` is mounted by the `Drawer` because the drawer owns the scroll container, so a second instance called from a page changes nothing : the shell's copy still fires and still scrolls `.drawer-content`. Without that forwarding the whole feature would have been unreachable to anyone using the library's shell rather than mounting their own `Drawer`.
- **It has a lab card at last**, on the pagination page — the case the hook was written for. It shows both halves of the rule : an ignored parameter that leaves the page where it is, and a watched one that lifts it. Both links pass `scroll={ false }`, so what is being watched is the hook and not Next. Measured from a scroll offset of 6000 : `?preview=` left it at 6000, `?page=` took it to 0.
