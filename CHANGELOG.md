...
- New **`useChartTheme`** (`hooks/`) building nivo's `theme` object from the active DaisyUI theme, replacing the prototype's `useNivoTheme`, which was written against DaisyUI 4 (`oklch(var(--bc))`) and produces invalid CSS under DaisyUI 5 — where `--color-base-content` already holds a complete color.
...
- New **`useChartPalette`** and `themes/charts/palettes.js`: `brand` (default, derived from the theme's `primary` by hue rotation with lightness/chroma modulation), `theme` (DaisyUI semantics), `nivo` (the prototype's look), or an explicit array. **`brand` switches to a hand-tuned palette past 6 series**, with a dev warning: measured minimum ΔE across all pairs drops to 10.0 at 6 series and 10.4 at 8 — and nivo's own scheme is worse (7.4). Distinguishability comes from hue spread, which is exactly what walks away from brand hues, so the two goals cannot both hold past ~5 series. Interpolating `primary → secondary → accent` was measured at ΔE 5 for 8 series and rejected: a smooth gradient is right for *sequential* data, wrong for *categorical*.
...
- New **`getChartLegends`** (`themes/charts/legends.js`) reducing nivo's dozen positional fields to `legend={ position }`. Placement is derived from the **already-computed margin**, so the legend always lands in the room reserved for it. Item colors are left unset so nivo inherits `theme.legends` — the prototype's hardcoded `itemTextColor: '#999'` disappeared in the dark theme.
...
- New **`ChartFrame`** owning the sizing box (`height` or `aspect`): nivo's `Responsive*` components measure their parent, and a parent with no resolved height measures zero — hence the `<div className="w-full h-[500px]">` sprinkled through the prototype.
